...
export VERSION=$(curl -s "https://api.github.com/repos/natrontech/container-images/releases/latest" | jq -r '.tag_name')
...
```bash
IMAGE=ghcr.io/natrontech/container-images:$VERSION

# get the image digest and append it to the image name
#   e.g. ghcr.io/natrontech/container-images:latest@sha256:...
...
cosign verify \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  --certificate-identity-regexp '^https://github.com/natrontech/container-images/.github/workflows/release.yml@refs/.*$' \
  $IMAGE | jq
```
