...
- NIC `5.6.0` → `5.6.1` (NGINX OSS 1.31.4, unchanged) — release notes: [v5.6.1](https://github.com/nginx/kubernetes-ingress/releases/tag/v5.6.1) (bugfix release: path quoting on v1 Ingress)
- all other components unchanged (already latest): coraza-nginx `v0.21.0`, libcoraza `v1.7.0`, OWASP CRS `v4.29.0`, Go `1.27`, Alpine `3.24` — no base-image digest drift
